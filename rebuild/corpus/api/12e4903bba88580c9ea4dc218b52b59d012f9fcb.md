# ARTICLE_REMOTE_CONTROL_FUNCTION Property (MDPartsDatabaseItemPropertyList)

ARTICLE\_REMOTE\_CONTROL\_FUNCTION Property (MDPartsDatabaseItemPropertyList)

Remote control function # 26041.

Overload List

| Overload | Description |
| --- | --- |
| [ARTICLE\_REMOTE\_CONTROL\_FUNCTION](Eplan.EplApi.MasterDatau~Eplan.EplApi.MasterData.MDPartsDatabaseItemPropertyList~ARTICLE_REMOTE_CONTROL_FUNCTION().html) | Remote control function # 26041. |

Remarks

Specification of the possibility of a radio remote control.

See Also

#### Reference

[MDPartsDatabaseItemPropertyList Class](Eplan.EplApi.MasterDatau~Eplan.EplApi.MasterData.MDPartsDatabaseItemPropertyList.html)
  
[MDPartsDatabaseItemPropertyList Members](Eplan.EplApi.MasterDatau~Eplan.EplApi.MasterData.MDPartsDatabaseItemPropertyList_members.html)

---

Eplan API, 18.03.2026, Â© by EPLAN GmbH & Co. KG. All rights reserved.

[Use the Eplan Global Support Portal for feedback and support](https://www.eplan.de/services/eplan-global-support/)