# ARTICLE_TORQUE_AT_MIN_SPEED Property (MergedArticleReferencePropertyList)

ARTICLE\_TORQUE\_AT\_MIN\_SPEED Property (MergedArticleReferencePropertyList)

Torque (at min. rotation speed) # 26251.

Overload List

| Overload | Description |
| --- | --- |
| [ARTICLE\_TORQUE\_AT\_MIN\_SPEED](Eplan.EplApi.DataModelu~Eplan.EplApi.DataModel.MergedArticleReferencePropertyList~ARTICLE_TORQUE_AT_MIN_SPEED().html) | Torque (at min. rotation speed) # 26251. |

Remarks

Torque at the minimum permissible output speed of the drive engine.

See Also

#### Reference

[MergedArticleReferencePropertyList Class](Eplan.EplApi.DataModelu~Eplan.EplApi.DataModel.MergedArticleReferencePropertyList.html)
  
[MergedArticleReferencePropertyList Members](Eplan.EplApi.DataModelu~Eplan.EplApi.DataModel.MergedArticleReferencePropertyList_members.html)

---

Eplan API, 18.03.2026, Â© by EPLAN GmbH & Co. KG. All rights reserved.

[Use the Eplan Global Support Portal for feedback and support](https://www.eplan.de/services/eplan-global-support/)