...
- The part contains a device definition (function templates).
...
Depending on the copy mode, a DT is assigned to new devices. (You define the copy mode in the project settings: File > Settings > Projects > "Project name" > Devices > Numbering (online).)

If a device is placed on a Panel layout page type, then a new part placement (and not a component) is created.

### Define the main function

To allow the part to be transferred, a main function must be placed. If a device is placed that does not contain a main function, then the first placed component automatically becomes the main function.
...
- If function templates are entered for the part in addition to the macro, they are displayed in the navigators and overlaid with the matching functions from the macro variants. If at least one function template matches a macro variant, then all other functions from this variant are also generated as unplaced functions.  
...
If both a macro and function templates are entered at a part, functions contained in other macro variants of this macro can also be considered when you insert a device in the graphical editor (via the Insert center). This depends on the project setting Consider macro when generating devices.
