# Tab Connection points (connection point pattern)

[![Closed](../../Skins/Default/Stylesheets/Images/transparent.gif)Dialog call:](javascript:void(0);)

Tab Master data > Command group Parts > Management. You have selected an object in the tree that belongs to the Connection point pattern hierarchy level. Select the Connection points tab.

In this tab you define a collection (i.e. a group) of connection points. Here you specify the spatial arrangement and specific data of the connection points that are part of a connection point pattern.

Overview of the main dialog elements:

Scheme:

Choose the scheme that you would like to use for the display of the connection point data from the drop-down list. Or click [...] to edit an existing scheme or create a new one in the [Settings: Column configuration](modaldialogsdb_d_schemaspaltenkonfiguration.htm) dialog.

---

Table

The most important connection point data of the table are listed below.

Bus interface name:

In the case of bus ports the bus interface name is additionally used for identification.

Plug designation:

Enter the DT of the plug used to connect the connection point here. The plug designation supports the identification of PLC connection points or device connection points.
...
You determine the coordinate values of the connecting points in the columns X position to Z position.
...
You determine the values for the connection point vector with which the routing connection exits the connection point In the columns X vector to Z vector. The values describe the preference with which the vector points to the direction of the respective axis.
...
In this column you specify the direction in which the routing connection is to preferably search for an entry into the routing path network after the exit from the connection point.
...
From this drop-down list select the default value that specifies the connection point type of the connections. The corresponding entries are made in the [Settings: Connection category](partsmanagementgui_d_einstanschlussausf.htm) dialog. This dialog can be reached via the File > Settings > Company > Management > Connection category commands.

We recommend using only the [connection categories corresponding to the ECLASS classification](partsmanagementgui_d_einstanschlussausf.htm#Tabelle) when new connection point patterns are created.
...
[Tab Properties (Hierarchy level "Data" (for connection point patterns))](articlesgui_r_eigenschaften.htm#DatenAnschlussbild)