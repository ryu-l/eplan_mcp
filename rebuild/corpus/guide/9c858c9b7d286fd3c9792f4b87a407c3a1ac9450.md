...
Please note that the functions grouped into a device group are treated as a group only when the [Consider groupings](gededitgui_h_elementegruppieren.htm#I_GruppierungBeruecksichtigen) option has been activated. Otherwise, the functions are not treated as a group but as individual functions with actions like block editing, deleting, copying, moving, etc.
...
- If you selected the "Function properties 1" element in the child property group, navigate via the ![](../Pictures/Gui/ALL/all_stairup_as.png) (Next level) button to the indirect properties of a different function of the device group. If at that time you are in the property dialog of an auxiliary function, the values of the main function will be output as a result.
