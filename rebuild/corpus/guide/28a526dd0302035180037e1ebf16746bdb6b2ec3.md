...
You have opened the project whose parts are to be synchronized; you have not opened an editor for editing master data (plot frame editor, form editor ...). Tab Master data > Command group Parts > Synchronize.
...
- Variant: Variant designation of the part
- Last change: Date of the last change of the part in the project.

The entries are sorted in alphanumeric order in columns. By default they are sorted by the Part number entry. If you click the Variant column, all the entries are shown sorted by a variant designation.
...
These entries are also sorted in alphanumeric order in columns. By default they are sorted by the Part number entry. If you click the Variant column, all the entries are shown sorted by a variant designation.
...
- If a part is present in the project, but not in the system, synchronization from the project to the system is possible, but the part will then be created in the system.   
  If a part is present in both the project and the system, and if the part in the project is "newer" than the system part, no data is transferred to the system. For this reason, the ![](../Pictures/Gui/ALL/all_arrowright_as.png) button is grayed out for a single selection. The Complete current system menu item remains active, however, since this affects the entire dialog.
