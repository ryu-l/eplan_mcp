...
You have opened a project. You have opened the layout space navigator and a layout space. The layout space contains part placements (3D). You have double-clicked the part placement or have selected the Properties popup menu item. In the Properties <Component>: Part placement (3D) dialog, select the Connection point pattern tab.
...
Note that the Stripping length in the connection point pattern must only be used for the output of connection lists and for labeling purposes but not for the manufacturing data export via the Komax wire fabrication machine. If this export is to be used, the values of the stripping length have to be entered explicitly in the export settings for the wire fabrication data of this machine. In the settings dialog for the Komax machine the [Wire termination processing](productionwiringgui_r_verbindungsendebehandlungkomax.htm) tab is available for the specification of the stripping length.
...
| Menu item | Meaning |
| --- | --- |
| Adapt connection point pattern | Generates a local connection point pattern. In the process contradictory connection points that are identified with the icon  or  are removed from the connection point pattern if the same contradictory connection points are also available at the placed device in the schematic. |
