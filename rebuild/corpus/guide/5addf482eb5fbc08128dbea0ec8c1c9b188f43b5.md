...
You have opened a project. File > Settings > Projects > "Project name" > Graphical editing > General.
...
This setting only has an effect if during the preparation of the macros, the option "from project settings" was selected for the setting [Also insert macro box](macrosgui_r_makrokasteneinstellungen.htm#MakrokastenEinfuegen) in the macro project at the respective macro box.
...
- Please note that the behavior described here only applies for general functions for the search for a path function text. For terminals, shields, PLC connection points, etc. the search functions slightly differently. For further information refer to the section ["Reporting of Path Function Texts"](gededitgui_k_pfadtextauswertung.htm).
...
B) With a user-defined property arrangement: the property texts are moved manually.
...
If this check box is activated, the Do not use anymore setting is assigned to the new user-defined properties in the [configuration dialog](eservicesgui_d_konfigeigenschaften.htm). These properties are then not visible at the assigned objects in the project and are deleted when the project is compressed again.
...
Select the background colors for the shading of the model views by clicking [...] and opening the [Color selection](mf_d_farbauswahl.htm) dialog.
...
From the drop-down list select the coding to be used for inserted QR codes. The QR code is encoded in UTF-8 by default. The "Shift JIS" coding is available for Japanese characters.
...
[Inserting QR Codes](gededitgui_h_qrcodes.htm)