...
These functions get their data from the macro or â if the macro contains placeholder objects â from the value sets that are assigned to the variables. The variables are not intended for assigning values to PLC functions, these receive their values when they are used.
