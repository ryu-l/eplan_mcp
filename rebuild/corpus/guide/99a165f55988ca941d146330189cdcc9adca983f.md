...
- If accessory lists are used when adding accessories via device selection, you can also insert the accessory lists completely. To this purpose the Insert complete accessory list property has to be activated in the parts management for the respective accessory list.
...
If you carry out a device selection for an empty PLC box, for example, only parts that have a function template with the function definition "PLC box" (and any number of function templates for the PLC connection points) will be displayed.
...
- Module (multiple symbols, different device tags), for example star-delta starter circuit as a 19" / MCC insert.
