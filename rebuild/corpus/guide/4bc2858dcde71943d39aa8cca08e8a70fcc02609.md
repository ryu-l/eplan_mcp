...
You have opened a project. Tab Pre-planning > Command group Edit > Configure segment definitions.
...
On the left in the dialog, the user-defined segment definitions that exist in the current project are displayed in a tree view. The basic segment definitions are displayed as the top hierarchy level; below that, the segment definitions are displayed with their displayed names. If you have not created any segment definitions of your own yet, only the predefined ones will be displayed. You can select multiple segment definitions. Some functionalities â such as the creation of new segment definitions or the configuration of the segment properties â are not possible at a multiple selection and are grayed-out in such a case.

The following basic segment definitions are available:

- Structure segment: Here, you can create the segment definitions for structure segments. The "Structure segment general" segment definition is predefined.
- Planning object: Here, you can create the segment definitions for planning objects. The "Planning object general" segment definition is predefined.
- PCT loop: Here, you can create the segment definitions for PCT loops. The segment definitions "Loop" and "Consumer" are predefined.
- PCT loop function: Here, you can create the segment definitions for PCT loop functions. The segment definitions "Measuring function" and "Consumer function" are predefined.
...
- Piping planning object: Here, you can create the segment definitions for piping planning objects. The "Piping planning object general" segment definition is predefined.
- Cable planning object: Here, you can create the segment definitions for cable planning objects. The "Cable planning object general" segment definition is predefined.
...
If this check box is enabled, only segments that are marked as "Do not use anymore" are displayed in the tree view. For these segments, the Do not use anymore check box has been enabled on the Configuration tab.
...
Depending on the selected category, the user-defined pre-planning properties defined in the [Configure properties](eservicesgui_d_konfigeigenschaften.htm) dialog (accessible via the Configure popup menu item) are displayed in the table. The ![](../Pictures/Gui/ALL/all_new_as.png) (New) button allows you to add more properties. You can assign these properties to the segment definitions, and thus display them in the project as well as output them in reports.
...
- Write protection: Here, you specify whether the value of the property may be modified (entry "No") or is write-protected (entry "Yes").  
  If the setting Write-protected on the user interface was activated for a user-defined property in the dialog Configure properties, the write protection can no longer be modified here.
...
- When special basic projects are used, for example for building automation, predefined user-defined properties are available to you.
...
By means of the write protection you can configure the segment definitions of the structure segments so that only the structure identifiers of one identifier block (for example, function designation, location designation, etc.) can be inserted and edited per structure level in the pre-planning navigator.

![](../Pictures/Gui/ALL/example.png)[![Closed](../../Skins/Default/Stylesheets/Images/transparent.gif)Example:](javascript:void(0);)

In a project the identifier blocks "Function designation", "Location designation" and "Higher-level function number" and among other things the "Works" structure level. The Write protection check box is activated at the identifier blocks "Location designation" and "Higher-level function number" in the Write protection and numbering for structure identifier table for the segment definition of this structure segment. This means that a pre-planning user can then only enter structure identifiers for the "Function designation" identifier block at the structure segments of the "Works" structure level.

For the other segment definitions you can configure a write protection for the individual components of the structure identifiers (such as the property Function designation (single component) at planning objects, PCT loops, etc.).

The table contains the following columns:

- Structure: Lists the identifier blocks which correspond to the project structure.
...
- Number: Only available if the write protection is not activated. Here you specify how numbering is to be carried out for the structure identifiers of the respective identifier block and for the individual component of the symbolic address (Do not number, Consecutive, Number of the designation, From designation).

| Entry | Meaning |
| --- | --- |
| Consecutive | Increments the counter. |
| Number of the designation | The structure identifiers or the symbolic address contain the "Number" component of the designation and are numbered according to the settings in the Designation numbering group box. |
| From designation | The structure identifiers or the symbolic address adopt the full designation of the segment. Existing values are overwritten in the process. When numbering the "Number" component, the settings in the Designation numbering group box are taken into account. |
