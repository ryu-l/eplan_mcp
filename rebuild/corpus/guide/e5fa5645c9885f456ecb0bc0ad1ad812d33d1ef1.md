...
For the PLC data exchange in the AutomationML AR APC format the network structure to be replaced is determined from the single-line or multi-line bus ports connected to each other. For all other formats only the single-line representation is considered.

Multi-line representation of the bus is not required. Detailed multi-line representations of standard bus cables can be implemented by means of a cable assignment diagram.
...
The single-line variant of a part macro is shown on the left and the multi-line variant on the right.
...
1. Select the following commands: Tab Insert > Command group Devices > ![](../Pictures/Gui/ALL/ribbon_busport_as.png) Bus port.
