...
Cross-references are also possible between devices when you have placed them on multi-line and single-line schematic pages. To this purpose you must use File > Settings > Projects > "Project name" > Cross-references / contact image > Display to inform Eplan that device cross-references between the two types of representation are to be created and displayed.
...
For every device, you can define whether a contact image or a cross-reference is to be displayed. The Contact image tab is available in the Display tab of the properties dialog below the property arrangement for this purpose. If you select one of the contact image alignments "In path" or "On component" via ![](../Pictures/Gui/ALL/all_new_as.png) (New), a contact image is displayed according to the settings. You can edit the settings for the display of the contact image in the Property / Assignment table in the right-hand section. If you do not select a contact image alignment, only one cross-reference list is displayed.
...
The cross-reference is identified by a special prefix, defined in the project-specific settings under File > Settings > Projects > "Project name" > Cross-references / contact image > General.

To make the device cross-references visible at main functions, you must use File > Settings > Projects > "Project name" > Cross-references / contact image > Display to enable the cross-reference display between the page types "Multi-line representation" and "Device tag list". Furthermore the Cross-reference display property has to be set to "Always display" at the individual main function in the Properties <...> dialog.

### Prefix for cross-references

Due to the large number of cross-reference types, which can also be displayed in combination at the component, Eplan allows the various cross-reference types to be visually distinguished from each other by defining a specific prefix for each type. The prefix can be a particular character or string of characters and can be defined for different page types (multi-line schematic page, overview, etc.). The settings are project-specific and are specified under File > Settings > Projects > "Project name" > Cross-references / contact image > General in the table with the columns Page type / prefix.
