...
- Dynamic forms: With these forms, any graphics that may exist are dynamic, and you have the opportunity to insert so-called "dynamic areas". The row height can be varied by activating the Adapt row height dynamically form property, and it is possible to combine the reports of several functions on one report page.

### Dynamic areas

In the following sections the individual dynamic areas that you can insert into a dynamic form are explained (command path in the form editor: Tab Insert > Command group Text > Drop-down button Forms > ...).
...
If you want to have a data header area output for each function designation, the report must be sorted by function designations. This is the only way to ensure that each function designation data header area is only output once.
