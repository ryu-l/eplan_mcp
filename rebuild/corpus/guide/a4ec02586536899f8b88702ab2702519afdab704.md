...
You have opened a project. You have selected a function in the graphical editor or in a navigator. Popup menu item Properties. In the Properties <...> dialog, select the Symbol / function data tab and click [Logic].
...
- Conductor / wire: The connection is either a conductor in a cable (then the check box Cable connection point must be selected) or a single connection (wire).
...
- Direct connection point: Direct connection between items, e.g. if a PLC card is plugged into the rack or a fuse on a rail or the connection of a male and female pin.
- Optical fiber: The connection can be part of a hybrid cable. In reports, you can filter according to this type of connection point.
...
- Functional connection point (software): Functional connection points serve to represent the logic of PLC configuration programs and are not output in reports.
...
- Mechanical feedthrough: With two connection points of this connection point type, the connection is routed through the device for current transformers with [mechanical feedthrough](egedgui_k_stromwandler.htm).
...
Through this property you can control the source and target of cable connections, if the Determine source and target of the cables via the cable connections project property is activated. You can find further information in the section "[Page-oriented Determination of Cable Source and Target](cablegui_k_prinzip.htm#I_SeitenorientierteErmittlungQuelleZiel)".
...
Since terminals have no plug designations, an entry at distributed terminals such as "X2:2" is evaluated like a simple connection point designation.
