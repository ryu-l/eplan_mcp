# Eplan Fluid: Routing of Fluid Connections
...
- Assign 3D graphical macros:
  ...
  To obtain a realistic and detailed display of the prototype in the 3D mounting layout, you should generate 3D graphical macros for your fluid power items (valves, pumps, etc.) and assign them to the parts. Macros with 3D data and the "3D mounting layout" representation type are stored in the parts management on the Properties tab in the Graphical macro property.
- Define connection point patterns:
  ...
  The parts management provides a special main node for the definition of a connection point pattern with the associated data of the connection points. In the Properties tab of a part, you can use the Connection point pattern property to select the desired connection point pattern and store it with the part.
...
- Create part for routing accessories:
  ...
  Similar to the other 3D components to be placed in the layout space, you must have stored the corresponding parts for placing routing accessories in parts management.
- Enter connection data for tubes:
    
  For the correct display of the routed tubes in the layout space, the required connection data (color, outside diameter) must be defined. The entries for this can be made in the properties dialog of the connection or at the part in parts management.

![](../Pictures/Gui/ALL/example.png)[![Closed](../../Skins/Default/Stylesheets/Images/transparent.gif)Example:](javascript:void(0);)

You have opened the properties dialog for a connection and brought the Connection tab to the front. In the Color / number field of the properties dialog, enter the color of the tube (e.g. BU for blue). Enter the dimensions of the tube in the Cross-section / diameter field (e.g. Ã10 or Ã10x1.5 for a tube with an outside diameter of 10 mm and a wall thickness of 1.5 mm).
...
The free routing of connections via the Free routing (layout space) command generates curved routing connections outside of a routing path network. Source and target devices are automatically interconnected in the shortest path possible.

The Route (layout space) command is provided for electrical connections that are routed within a routing path network.

### Adjust connection form

To adapt the curve run of a freely routed connection to the spatial conditions in the mounting layout, you predefine a connection form by means of curves and routing paths. Using the Change route command, you position the automatically routed connections in the desired routing track.

In special assembly situations of devices, it may be necessary to change the routing direction of all connection points specified in the connection point pattern in order to avoid unfavorable routing tracks. This can be done by means of the Change routing direction command.
