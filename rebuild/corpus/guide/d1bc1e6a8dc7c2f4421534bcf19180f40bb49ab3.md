...
You have opened a project. You have opened a layout space. In the command group Devices of the Insert tab you have selected a part for placement (for example a mounting panel). During the placing you select the popup menu Placement options.
...
The handles for part placements with a simplified representation (cuboid generated from part dimensions) are at the corner points on the base of the object.
...
This check box is only available when items with the "Mounting panel" and "Enclosure" function definition are placed. The check box can be used to specify how mounting panels and also complete enclosures are represented in the hierarchy of the layout space navigator. This setting thus also influences the behavior during the displacement of parts and for reports, for example mounting lists through the set property Item is immovably attached to the superior item.
...
At mounting panels the Item is immovably attached to the superior item property is deactivated temporarily, if moving was carried out by using the tab Edit > Command group Graphic > Move or by using the [V] key. The mounting panel can then be moved with all the items placed on it.
...
Using this drop-down list, you specify what the distance set in the Distance field references.
...
- Center distance: The distance information relates to the distance from object center to object center.
