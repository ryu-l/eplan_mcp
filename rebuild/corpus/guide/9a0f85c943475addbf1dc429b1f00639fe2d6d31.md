...
If the "From project settings" option was activated in the Also insert macro box setting during the preparation of the macro, you can specify project-wide for a schematic project that macro boxes are also to be inserted. To do this, activate the [Also insert macro boxes](gedviewer_d_einstellungenprojektallgemein.htm#MakrokaestenMitEinfuegen) project setting.

![](../Pictures/Gui/ALL/note.png)Note:

When compressing projects, there is a setting available for deleting all of the inserted macro boxes in a project. To do this, select the Macro boxes check box (in the Remove placed functions hierarchy level) and execute the subsequent compression (see also [Compressing a Project](projectcleangui_h_projektsaeubern.htm)).
