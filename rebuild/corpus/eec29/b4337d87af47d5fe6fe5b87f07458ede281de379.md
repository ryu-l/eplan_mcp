...
Typically, the option -configuration is used in combination with -data. If the configuration directory and the workspace are modified at the same time, an error message will result: [Class](glossary_o_class.htm) not found. An error-free migration requires two steps: 1. Only convert the workspace to the new directory. Then, convert the configuration.
