...
A folder in which the Job definitions are stored has to be specified in the user specifications for each Job Server Worker (see [Preferences for the Job Server Worker](admin_r_prefs_js_worker.htm)).
...
[Job Server with Job Server Worker installation](admin_h_installation_jobserver_worker.htm)

[Job Server Worker installation](admin_h_installation_worker.htm)