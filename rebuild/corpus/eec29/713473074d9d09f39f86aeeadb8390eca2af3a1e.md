...
- [Model variable](glossary_o_modelvariable.htm) without specification or a developer license is active: EPLAN support request is sent to the e-mail address [support@eplan.de](mailto:support@eplan.de).
...
- Discipline resources. This option is required if resources are not managed internally in the model (see [Resources and their management](eecbase_k_resources_and_their_management.htm)). The resource files are only attached if the global path to the resources is specified (see [com.mind8.global.resourcePath](admin_r_vmargs_global_resourcePath.htm)). This option is enabled by default.
...
[Call](eecbase_k_qualityfeedback_call.htm)

[Create and send](eecbase_k_qualityfeedback_create_and_send.htm)

[Data protection](eecbase_k_qualityfeedback_data_security.htm)