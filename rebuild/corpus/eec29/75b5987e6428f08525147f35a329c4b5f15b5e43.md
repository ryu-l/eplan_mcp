# Generate comments in calls

Path to model variable:

Disciplines > PLC > CoDeSys > Code Generator

Select a [Boolean](glossary_o_boolean.htm) value. The value determines whether block calls are provided with comments during the generation of the PLC code.

Valid values are true and false.

![](../Pictures/Gui/ALL/example.png)[![Closed](../../Skins/Default/Stylesheets/Images/transparent.gif)Example:](javascript:void(0);)

Block call with comments (Value = true):

```
Xaxis1(
	moveForward := forward1,
	moveBack := backward1,(* X-axis moves backward*)
	inFront := inFront1,
	inBack := inBack1,(* X-axis reached back position*)
	forward => forward1,
	Bus := Bus.No1(*Transportation*)
	);
```

Block call without comments (Value = false):

```
Xaxis1(
	moveForward := forward1,
	moveBack := backward1,
	inFront := inFront1,
	inBack := inBack1,
	forward => forward1,
	Bus := Bus.No1
	);
```