...
| [Parameter](glossary_o_parameter.htm) unit | [Unit](glossary_o_unit.htm) within the library where parameters are automatically created, as needed. If, for example, a resource with previously unknown parameters is inserted, EEC automatically creates new parameters in this unit. |
...
- **OpenHTMLWithEclipseCommand:** Opens the created HTML document in the editor specified in Eclipse (set via User preferences -> General -> Editors -> File associations)
