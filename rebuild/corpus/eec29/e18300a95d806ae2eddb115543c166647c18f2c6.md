...
- The functional assembly is used to create the abstract [Mod. system](tutorial_k_example_feeder_modular_system.htm).
...
- In the first step, the basic architecture is defined ([Create an architecture model](tutmechatronic_h_create_mechatronic_architecture.htm)).
- Building on this, the components required for the feeder are specified ([Create a mechatronic modular system](tutmechatronic_h_develop_mechatronic_modular_system.htm)).
...
- How it possible to use the plug-socket concept to create a list of components that differs in structure from the functional layer is demonstrated in the section [Mechatronic model: Generating non-functional structures](tuttext_h_generate_nonfunctional_structure.htm).
...
- The first step is to [Creating EPLAN Pro Panel discipline components](tutpropanel_h_create_propanel_discipline_components.htm) for this discipline.
...
- The test at the end of the section on [Setting a disabler for digital input cards](tutpropanel_h_set_disabler_for_inputs.htm) shows how a cabinet that is adapted to the installed components can be created with the aid of the disabler concept.

The PLC discipline is used to show how to create a program for [SIMATIC Step 7](tutstep7_h_step7.htm) based on the mechatronic model. This discipline is divided into four sections:

- The first section shows how a Step7 program is to be created for the Feeder example machine ([Task 1 - Creating the Step 7 program for Feeder](tutstep7_h_task1_generate_code.htm)).
...
- How it possible to use the plug-socket concept to create a list of components that differs in structure from the functional layer is demonstrated in the section [Generating non-functional structures](tutword_h_generate_nonfunctional_structure.htm).
...
- Container objects (floor, room) are defined as polygons and linked with Form-UIs to enable data to be entered ([Expanding the configuration with nodes for floor and room](tutgraph2d_h_extend_configuration_by_floor_and_room_nodes.htm) and [Configuration of tooltips for displaying Form-UIs](tutgraph2d_h_configuration_of_tooltips_for_formuis.htm)).
...
- Container objects, devices, persons, and connections are assembled in a tool palette ([Configuration of palette entries](tutgraph2d_h_configuration_of_palette_entries.htm)).
...
[Tutorial files](tutorial_k_files.htm)

[Comparison with sample solution](tutorial_h_compare_with_example_solution.htm)

[Example Feeder](tutorial_k_example_feeder.htm)
...
[EPLAN Electric P8](tutp8_h_eplan_electric_p8.htm)

[EPLAN Pro Panel](tutpropanel_h_propanel.htm)

[SIMATIC Step 7](tutstep7_h_step7.htm)

[Word](tutword_h_word.htm)

[GRAPH2D discipline](tutgraph2d_h_graph2d.htm)

[Project generation per import](tutimp_h_import.htm)

[Job Server](tutjobs_h_jobserver.htm)