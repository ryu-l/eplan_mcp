...
The target of the link is specified by the receiver attribute, and the ID of the Form-UI to be opened is specified by the ref attribute. A Tooltip appears in the pureTable when the mouse is moved over an entry in the âStationâ column. [Double](glossary_o_double.htm)-clicking next to the Tooltip but within the table cell opens up the linked Form-UI.
