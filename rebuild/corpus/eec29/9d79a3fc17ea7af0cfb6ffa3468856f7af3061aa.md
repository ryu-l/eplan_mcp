# addError(Object reportingInstance, String causeObjectID, String topic, String message)

Adds to the UMC an error of the LIBRARY type.

| addError(Object reportingInstance, [String](glossary_o_string.htm) causeObjectID, String topic, String message) | | | |
| --- | --- | --- | --- |
|  | Object | reportingInstance | The object causing the error and/or reporting the error. |
| String | causeObjectID | Currently null; is set aside for future extensions. |
| String | topic | The message title. Is used both as the title in the message window and the identifier for the logger. |
| String | message | The message text. |
| Return value | [Boolean](glossary_o_boolean.htm) | | true or false |