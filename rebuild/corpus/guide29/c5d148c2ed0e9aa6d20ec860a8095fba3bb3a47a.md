...
For certain [report types](Glossary_o_auswertungstypen.htm) (e.g. device tag list, [cable](Glossary_o_kabel.htm) assignment diagram, terminal-strip overview, etc.) it is possible to specify a next [form](Glossary_o_verlauf.htm). In [reports](Glossary_o_auswertungen.htm), the next form is output after the current form. The next form can be inserted into the same page as the current form or on a subsequent page, and you define this using the insertion point of next form.

### Insertion point of next form

An Insertion point of next form can be placed in a form in order to insert another report at this point. This allows the generation of [report blocks](Glossary_o_auswertungsbloecke.htm) composed of two different [forms](Glossary_o_formulare.htm). Together, all pages generated for a single form name define a report block.

It is also possible to insert several insertion points of [next forms](Glossary_o_folgeformulare.htm) into a form. The report sequence is determined by the graphics.
...
Depending on the form type of the opened main form, different report types are available for next forms. For most [form types](Glossary_o_formulartypen.htm), the "Summarized parts list" and the "Parts list" are available. "Identical form type" in the table means that, e.g., for the "Cable diagram" report type a next form of report type "Cable diagram" is possible.
...
| Topology: Routed cables / [connections](Glossary_o_verbindungen.htm) | x | x |  |  |
