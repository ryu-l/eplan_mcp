...
The relevant internal jumpers, saddle jumpers or wire jumpers are generated from all of the [connections](Glossary_o_verbindungen.htm) that run between [distributed terminals](Glossary_o_teilklemmen.htm) on a terminal strip when updating the connections. Internal jumpers are generated between associated distributed terminals (same function definition, same DT, same terminal designation, same level number); saddle or wire jumpers are generated between distributed terminals that are not associated.
...
All distributed terminals with the same DT, same, occupied terminal designation and different connection point designation are viewed logically as one terminal whose [connection points](Glossary_o_anschluesse.htm) are connected to each other.
