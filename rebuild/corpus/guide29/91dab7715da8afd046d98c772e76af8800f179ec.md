...
A terminal on a terminal strip was defined as the start of a switching jumper. However, none of the function [connection points](Glossary_o_anschluesse.htm) of the terminal in the connection point logic is an external connection point. Consequently, no switching jumper is created.
...
At the last terminal of a terminal strip, a closed switching jumper was defined in the Edit terminal strip dialog. Since the last terminal is not followed by an adjacent terminal, it is not possible to [create](Glossary_o_erstellen.htm) a switching jumper, which also means there is no jumper end.
...
4. Close the Connection point logic dialog and the Properties <...> dialog using [OK], and update the [connections](Glossary_o_verbindungen.htm).
...
2. You have extended the terminal to include the [properties](Glossary_o_eigenschaften.htm) Switching jumper (internal) and Switching jumper (external).
