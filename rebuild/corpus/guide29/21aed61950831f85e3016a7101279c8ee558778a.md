# Automatic Interpretation: Principle

Automatic interpretation transfers the existing logic of an item, such as an enclosure, to another enclosure of the same or similar design. The transfer of the item logic is not only possible with complete enclosures, but also with enclosure components, etc. that must be available in miscellaneous sizes to be used within a series. The objective is to minimize the expenditure of related to the creation of [master data](Glossary_o_stammdaten.htm).

The preparatory step is always the detailed manual assignment of the logic to the first enclosure of the design with the [functions](Glossary_o_funktionen.htm) of the device logic:

- Import of the 3D graphic via interfaces (e.g., STEP)
- Definition of the individual [items](Glossary_o_bauteile.htm) of the enclosure (function definition, layer, transparency, etc.)
- Definition of the placement area
- Definition of [mounting surfaces](Glossary_o_montageflaechen.htm) on the items.

The first enclosure of a design defined in this way will be used as a model for the automatic definition of the other enclosures of this design. You must keep in mind that this sample enclosure and subsequently defined enclosures must always be of the same design (e.g., AE, CM, TS8).

After the sample enclosure has been defined, Automatic interpretation is launched. An item definition "Enclosure" is generated from the items selected in the [layout space](Glossary_o_bauraum.htm). The following [properties](Glossary_o_eigenschaften.htm) are defined automatically:

- Function definition: Enclosure, general
- Item designation: Enclosure
- Transparency: From layer
- Color: From layer
- Layer: EPLAN560, 3D graphic.Enclosure.

Then, you must select the sample enclosure whose logic is to be transferred to the new item definition.

In the last step, the program analyzes the sample enclosure and the new enclosure, and defines automatically at the new enclosure the items including mounting surfaces as well as all other characteristics:

- Function definition
- Mounting surfaces
- Mounting surface (Change size)
- Field sizes
- Handles
- Mounting points
- Base points
- Mounting [grid](Glossary_o_raster.htm)
- Transparency
- Color
- Layer
- Apply superior [layers](Glossary_o_ebenen.htm)
- Item designation
- Item description
- Item [ID](Glossary_o_id.htm)
- Property arrangements
- Property Item is fastened unmovably to the superior item
- Property Item requires holes in mounting surface
- Property Create mounting list.

Limitations in the application of automatic interpretation can occur in case of different numbers of doors or significant differences in size between the model enclosure and target enclosure. In this case, some items may not be able to be assigned, so that these items may have to be defined manually at the target enclosure.

### Similarity check and matching results

With automatic interpretation, it is also possible to work without a clear assignment of a template and to let the similarity check decide which items are best suited as a template. If several templates have been selected in the Select interpretation template selection dialog, the similarity check starts and determines which templates have the most accurate similarity / highest matching results of certain characteristics compared to the 3D object to be defined:

- Quantity of items
- Size and position of individual items
- Description of the items in the imported STEP file.

The results are displayed in the Matching results dialog, including the percentage of similarity. Here, the most suitable item can be selected as a template.

See also

[Interpreting and Carrying over Enclosure Logic](cabinetgui_h_schrankinterpretation.htm)

[Dialog Select interpretation template](cabinetgui_d_musterschrankwaehlen.htm)

[Dialog Matching results](cabinetgui_d_trefferliste.htm)