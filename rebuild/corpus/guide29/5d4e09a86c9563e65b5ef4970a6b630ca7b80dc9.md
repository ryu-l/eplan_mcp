...
Calculating the source and target of a [cable](Glossary_o_kabel.htm) is done according to the following rules:

- First the identifying hierarchy specifications from the structure identifier are taken into consideration.
- If both ends are terminals or [plugs](Glossary_o_stecker.htm) and the identifier is the same, the smaller number is the source.  
...
Cable [connections](Glossary_o_verbindungen.htm) are sorted during the automatic assignment of the [part](Glossary_o_artikel.htm) connections. The sorting defines the sequence in which the [cable connections](Glossary_o_kabelverbindungen.htm) are assigned:
