...
- Overview of the [rack](Glossary_o_baugruppentraeger.htm) layout (macro variant E).
...
EPLAN generates a PLC schematic by automatically [placing](Glossary_o_auflegen.htm) [macros](Glossary_o_makros.htm) and using [unplaced functions](Glossary_o_nicht_platziertefunktionen.htm) from the project. These macros must be assigned to the PLC [devices](Glossary_o_betriebsmittel.htm), which largely occurs automatically.

### Assignment of macros

The assignment of macros to the PLC devices is effected in the Generate PLC schematic dialog (menu [items](Glossary_o_bauteile.htm) Project data > PLC > Generate PLC schematic). The Macro file table column is used to this purpose. Once PLC devices have been read into the Generate PLC schematic dialog, the macro file from the [main part](Glossary_o_hauptartikel.htm) (the first [part](Glossary_o_artikel.htm) that is not an accessory) is automatically entered at the PLC box.
...
When assigning a PLC macro, a check is made to see if all necessary macro [representation types](Glossary_o_darstellungsarten.htm) are present and that they match the device [functions](Glossary_o_funktionen.htm). The results of this check are shown in the Status table column. If the macro contains all the necessary data then a green icon ![](../Pictures/Gui/ALL/all_statusok_as.png) is displayed. In rows of the macros where data is missing (for example a particular representation type), a yellow icon ![](../Pictures/Gui/ALL/all_statusnotok_as.png) is displayed. If the macro is missing or does not match (e.g. it contains too few [connection points](Glossary_o_anschluesse.htm)) then a red icon ![](../Pictures/Gui/ALL/erroricon_error_systemmessages.png) is displayed.
...
With [PLC connection points](Glossary_o_sps_anschluesse.htm) you can specify [wiring](Glossary_o_verdrahtungen.htm) macros. The wiring macros can contain single-line and [multi-line representations](Glossary_o_allpoligedarstellungen.htm). Automatic assignment of the multi-line and single-line wiring of the inputs and outputs is not possible when the connection point is not yet placed. You must perform this assignment yourself, by specifying appropriate [window macros](Glossary_o_fenstermakros.htm) in the Generate PLC schematic dialog, at the connection points where wiring is to be inserted. A macro can occupy multiple connection points, e.g. all connection points of a channel.

Automatic assignment of the wiring macro occurs when the connection point is already placed and the macro box was also inserted, e.g. via a previous schematic generation. At every placed connection point, a search is made in the connection point direction for the handle of a macro. The handle of the wiring macro must lie either exactly on the connection point of the PLC connection point or on the next [grid](Glossary_o_raster.htm) point in the connection direction.
...
I/O overviews for inputs and outputs and rack layout overviews are distinguished by the macro [variants](Glossary_o_varianten.htm). In both cases, the macros are placed using the "Overview" representation type. Macro variant A is intended for I/O overviews and macro variant E is for rack layouts.
