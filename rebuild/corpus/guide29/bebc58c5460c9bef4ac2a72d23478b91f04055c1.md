...
You can edit the data for a PLC at every position in the project where this data occurs: Both on the schematic and on the overview page and in the [navigators](Glossary_o_navigatoren.htm). It makes no difference where you begin editing. Changing the hardware data (e.g., the connection point designation) in the schematic does not change this data on the overview page.

![](../Pictures/Gui/ALL/note.png)Note:

If the scale is changed on schematic pages, the [property texts](Glossary_o_eigenschaftstexte.htm) for the [PLC boxes](Glossary_o_sps_kaesten.htm) are visually modified (scaled) in exactly the same way as for other components.   
...
You can [create](Glossary_o_erstellen.htm) PLC boxes and [connection points](Glossary_o_anschluesse.htm) in the graphical editor or in the device or PLC navigators. You can use the graphical editor to insert PLC boxes or connection points into a schematic or overview page and then edit their [properties](Glossary_o_eigenschaften.htm). The previously mentioned navigators allow you to predefine PLC boxes or connection points with particular properties and then place them at a later time.

[PLC connection points](Glossary_o_sps_anschluesse.htm) can also be inserted without PLC boxes. They are assigned to a PLC box through entry of the DT.
...
For bus ports the bus interface name together with plug designation is identifying. If the same bus port exists in multiple [representation types](Glossary_o_darstellungsarten.htm) in the project, it is necessary that the bus interface name is entered in all representation types for global editing and for [reports](Glossary_o_auswertungen.htm). With the project correction you can transfer the bus interface names at all representation types of the same bus port by using the Synchronize bus interface names setting.

You can place the plug of a PLC card on a separate schematic page. The [connection point designations](Glossary_o_anschlussbezeichnungen.htm) of the PLC connection points can be transferred via synchronization to the [pins](Glossary_o_steckerkontakte.htm) connected to these. A corresponding setting is available to this purpose in the project correction.

You can also draw the PLC configuration, the power supply, and the I/O assignments on separate schematic pages. Automatic [cross-references](Glossary_o_querverweise.htm) between these displays are automatically created and displayed. You can also mix these displays and draw them together on a single page. In this case, a PLC card box is the main function where the cross-references to the other boxes is displayed.
...
You draw [PLC card overviews](Glossary_o_sps_kartenuebersichten.htm) on pages of type "Overview". A synchronization in the [form](Glossary_o_verlauf.htm) of cross-references occurs between the PLC card overview and the schematic pages. All of the PLC connection point data can be entered in both the overview and in the schematic.

You can draw a PLC card overview at any time, even when no PLC [functions](Glossary_o_funktionen.htm) have been placed in the schematic. In this case, the cards and PLC connection points are created and can be later re-used in the schematic.
...
In reports, it is possible to display the data of a connected PLC connection point for all [devices](Glossary_o_betriebsmittel.htm). For this, in [forms](Glossary_o_formulare.htm) you can use the properties offered when entering placeholder text for the "Connected PLC connection point" element. In EPLAN, the following PLC-specific reports can be generated:
...
When synchronizing properties between multi-line and [single-line representations](Glossary_o_einpoligedarstellungen.htm) and overview pages, automatically determined properties, e.g. the function text and the symbolic address, are not overwritten.
