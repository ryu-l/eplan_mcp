...
Black boxes are used for the grouping of components ([connection points](Glossary_o_anschluesse.htm), [functions](Glossary_o_funktionen.htm) or [devices](Glossary_o_betriebsmittel.htm)), where the allocation of a [component](Glossary_o_schaltzeichen.htm) to a device can occur in various ways:
...
If the scale is changed on schematic pages, the [property texts](Glossary_o_eigenschaftstexte.htm) for the [black boxes](Glossary_o_geraetekaesten.htm) are visually modified (scaled) in exactly the same way as for other components.   
