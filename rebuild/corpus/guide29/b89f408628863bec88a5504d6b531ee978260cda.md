...
The page [properties](Glossary_o_eigenschaften.htm) contains the property Report: Source project ([ID](Glossary_o_id.htm) 11067) so that you can see for a generated report page from which project the data for the report originates. The name of the source project is entered automatically when the report is generated.

### [ClosedProvisional report projects](javascript:void(0);)

One possible use is the creation of provisional [report projects](Glossary_o_auswertungsprojekte.htm) for daily use. This method reports on a provisional status of a schematic project. To do so, first select the schematic project which is to serve as the source project in the page navigator. Then [generate](Glossary_o_erzeugen.htm) the report project (e.g. via the Utilities > Reports > Generate report project menu [items](Glossary_o_bauteile.htm)). The header data of the selected project will then be copied and a report project without pages is generated. The correct source project is already entered in the report templates of the new report project. Subsequently, the reports are generated.
