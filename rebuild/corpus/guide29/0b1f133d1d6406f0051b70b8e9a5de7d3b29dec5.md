...
The so-called [configuration projects](Glossary_o_konfigurationsprojekte.htm) can be used to define subsets of PLC [devices](Glossary_o_betriebsmittel.htm) within an EPLAN project. All [PLC boxes](Glossary_o_sps_kaesten.htm), [bus ports](Glossary_o_sps_busanschluesse.htm) and I/O [connection points](Glossary_o_anschluesse.htm) assigned to the same configuration project belong together and represent a PLC configuration. In the process the I/O connection points are automatically assigned to a configuration project via the respective PLC box. The data of the resulting specified set of devices can be exchanged with the [PLC configuration programs](Glossary_o_sps_konfigurationsprogramme.htm) of various PLC manufacturers. Using the Configuration project property it is possible to export and import, for example, a complete PLC controller, different networks together, or parts of a network.

A configuration project can contain several [stations](Glossary_o_stationen.htm) and bus connection points. Stations are subsets of PLC devices within a configuration project and can contain several racks with PLC devices as well as one or several CPUs. The [rack](Glossary_o_baugruppentraeger.htm) and the CPU are represented in the project by PLC boxes.

For a successful [data exchange](Glossary_o_datenaustausch.htm) at least one CPU must exist on the main rack within a configuration project. A CPU is uniquely identified by specification of the CPU name in the [form](Glossary_o_verlauf.htm) [Configuration project].[Station ID].[CPU identifier].

### PLC configurations

Bus systems and bus ports must be represented graphically on single-line or multi-line schematic pages so that the [network structures](Glossary_o_netzwerkstrukturen.htm) are included during the PLC data exchange. For the PLC data exchange in the AutomationML format the network structure to be replaced is determined from the single-line or multi-line bus ports connected to each other. For all other [formats](Glossary_o_formate.htm) only the single-line representation is considered. "Bus connection points" are [PLC connection points](Glossary_o_sps_anschluesse.htm) with the "Network / bus [cable](Glossary_o_kabel.htm) connection point" function definition. With the exception of PLC [functions](Glossary_o_funktionen.htm), general functions (e.g. converters, motors, valves, etc.) can be [part](Glossary_o_artikel.htm) of a bus system or network.
...
You want to configure a motor with a bus port. To this purpose you place a [component](Glossary_o_schaltzeichen.htm) for the motor and one for the bus port and assign the same DT to both of them. In this case the motor is the main function.

You want to configure a power converter with four [device connection points](Glossary_o_geraeteanschluesse.htm) and a bus port. To this purpose you place the corresponding component in a black box. The black box in this instance is the main function.

PLC devices are always to be configured as PLC boxes. These are:

- Devices with I/O connection points
- Devices that, in addition to the bus data, require further PLC [settings](Glossary_o_einstellungen.htm) (PLC structure data), for example CPU, power supply units, I/O cards, communication [modules](Glossary_o_module.htm), switches, etc.

![](../Pictures/Gui/ALL/info.png)Tip:

If you want to display certain [properties](Glossary_o_eigenschaften.htm) (for example the bus [ID](Glossary_o_id.htm)) of the single-line bus ports at the associated PLC box or device, you can use the [block properties](Glossary_o_blockeigenschaften.htm) to this purpose. When editing the format properties belonging to a block property the [Format](eservicesgui_d_format.htm) dialog provides the format element Network / bus cable connection point (single-line) of the device with the index values 1 to 5. This format element allows the data of the first five single-line bus ports of the device to be accessed and these to be displayed at the PLC box / device in the case of PLC devices bus-capable devices.

### PLC cards

A bus system or a PLC usually contains several [PLC cards](Glossary_o_sps_karten.htm), e.g., digital input or output modules. These are represented on the schematic by PLC boxes; a card can also be represented in distributed form using several boxes. In this case the PLC card data must be entered at the PLC box declared as the main function. You can also enter data at the other boxes, but this is not taken into account in [reports](Glossary_o_auswertungen.htm).
...
In the drive-oriented view of the PLC navigator, the PLC data within the [project structure](Glossary_o_projektstruktur.htm) is assigned according to the drives. The Drive property is identifying at the display in the tree view and therefore in one language.
