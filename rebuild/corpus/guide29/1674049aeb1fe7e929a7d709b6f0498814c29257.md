...
If [functions](Glossary_o_funktionen.htm) are linked with a planning object, or [planning objects](Glossary_o_planungsobjekte.htm) with linked functions are moved within the structure, the DT of the functions are numbered automatically. This happens with the following [actions](Glossary_o_aktionen.htm):
...
The functions linked with the planning [objects](Glossary_o_objekte.htm) are numbered according to the [settings](Glossary_o_einstellungen.htm) that apply to [online numbering](Glossary_o_onlinenummerieren.htm) when inserting symbols. In this case, no distinction is made between whether the function was placed as a symbol and whether it was placed from a macro. If the macro contains further functions not linked with a planning object, these functions will be numbered according to the settings for online numbering when inserting [macros](Glossary_o_makros.htm).

The [structure identifiers](Glossary_o_strukturkennzeichen.htm) of a function are compiled from the structure identifiers of the planning object and the target page.

A [product aspect](Glossary_o_produktkennzeichen.htm) entered at the structure segment is always [part](Glossary_o_artikel.htm) of the identifier structure. If no product aspect is entered at a subordinate planning object, it will be added during numbering.
...
[PLC connection points](Glossary_o_sps_anschluesse.htm) belonging to an address entry of a planning object on the basis of the (same) symbolic address and CPU are not numbered. But if the PLC connection point has a reference to a planning object, it, too, will be numbered in relation to it. (PLC [connection points](Glossary_o_anschluesse.htm) have a reference to the planning object only if their main function, that is, the PLC box, is defined in this planning object.)
...
In [offline numbering](Glossary_o_offlinenummerieren.htm), first the functions are numbered that are linked to a planning object, followed by the other functions.
