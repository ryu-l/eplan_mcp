# Tab Drill exceptions (NC Export All Machines)

[![Closed](../../Skins/Default/Stylesheets/Images/transparent.gif)Dialog call:](javascript:void(0);)

- Options > Settings > Company > Machining > "Export format". Select the Drill exceptions tab.
- Utilities > Manufacturing data > Machining > "Export format". Click [...] next to the Machine field. Select the Drill exceptions tab.

Use this tab to define drill exceptions. To define a new drilling exception, use a table to define the original drill hole that is to be replaced by a replacement drill hole. These programs can be used as replacement [drill holes](Glossary_o_bohrungen.htm) on machines running subprograms with special working steps (for example circular milling).
...
File name of a [user-defined](Glossary_o_benutzerdefiniert.htm) outline of the original drill hole.
...
To control the subprograms for circular milling (e.g., M20) on a Steinhauer machine, the following [settings](Glossary_o_einstellungen.htm) must be made on the Drill exceptions tab:
