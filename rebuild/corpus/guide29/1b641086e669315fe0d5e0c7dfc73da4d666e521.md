...
You have opened a project. Options > Settings > Projects > "Project name" > Routing connections > General. Select the Route tab.

Use this tab to define [settings](Glossary_o_einstellungen.htm) that affect the length calculation. As well, you define the limits for the display of the fill capacity of [wire ducts](Glossary_o_verdrahtungskanaele.htm).
...
The extra length for [wiring cut-outs](Glossary_o_verdrahtungsdurchbrueche.htm) is taken into account in calculating the length of a connection. With each feed-through of a connection through a [wiring](Glossary_o_verdrahtungen.htm) cut-out, such a length will be added. The default value is 2 mm.

Minimum bending radius included in length calculation:

If this check box is active, the "Minimum bending radius" [part](Glossary_o_artikel.htm) property defined for [wires](Glossary_o_draehte.htm), cables, and [connections](Glossary_o_verbindungen.htm) will be taken into account when calculating the length of [routing connections](Glossary_o_verlegeverbindungen.htm). For each 90° bend on the routing track, a length reduction is calculated and deducted from the overall length determined.
...
Using this setting, you can specify the connection cross-section from which a routing connection should no longer enter the routing path network in an orthogonal, but diagonal manner. This prevents bending positions, and it is possible to [route](Glossary_o_verlegen.htm) also connections with larger cross-sections without taking into account bending radii.
...
- By maximum routing track: With this calculation method, the defaults for the connection point direction and geometric position from the By connection point position (automatic) calculation method are applied to [connection points](Glossary_o_anschluesse.htm) with odd and even or alphanumeric [connection point designations](Glossary_o_anschlussbezeichnungen.htm). In addition, several [routing tracks](Glossary_o_verlegewege.htm) are calculated and compared to each other that result from the alternative consideration of the [connection point directions](Glossary_o_anschlussrichtungen.htm) "Up" and "Down" of the source and target of the connection. The greatest value calculated in this comparison is used as the length of the routing connection and entered at the routing connection.  
