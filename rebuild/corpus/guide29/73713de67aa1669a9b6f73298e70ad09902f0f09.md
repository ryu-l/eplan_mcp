...
The following [connection symbols](Glossary_o_verbindungssymbole.htm) are used in EPLAN to display direction changes and connection line junctions in the schematic:
...
When selecting T-nodes, double junctions, jumpers, or [interruption points](Glossary_o_abbruchstellen.htm), you have to specify the direction of the [target tracking](Glossary_o_zielverfolgung.htm) for the automatic report. Target tracking only treats [symbols](Glossary_o_symbole.htm) that represent [devices](Glossary_o_betriebsmittel.htm) as targets. It starts from one device and tries to find all of the devices along the automatically generated [connections](Glossary_o_verbindungen.htm). By using angles, T-nodes, double junctions, and jumpers the connection will not necessarily be in a straight line.
