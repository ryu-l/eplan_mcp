...
- Insert > Graphic > QR code
- Insert > Dimensioning > "Dimensioning type".
- Insert > Special text > Project property or Insert > Special text > Page property.
- Insert > Path function text.
- You have selected a text, a special text, or a path function text and the menu [items](Glossary_o_bauteile.htm) Edit > Properties or have double-clicked a text, a special text, or path function text.
- You have selected a hyperlink, a dimensioning, or an image file and have selected the menu item Edit > Properties.

Select the Format tab in the [property dialog](Glossary_o_eigenschaftendialog.htm) / subsequent dialog respectively.
...
- Insert > Special text > Project properties or Insert > Special text > Page properties, or Insert > Special text > Form properties.
- You have marked a text, an image file, or a dimensioning and have selected the menu item Edit > Properties.
...
- Insert > Graphic > Image file.
- Insert > Dimensioning > "Dimensioning type".
- Insert > Special text > Project properties or Insert > Special text > Page properties, or Insert > Special text > Plot frame properties.
- Insert > Special text > Column text or Insert > Special text > Row text.
- Insert > Special text > Watermark (revision control).
- You have marked a text, an image file, or a dimensioning and have selected the menu item Edit > Properties.
- You have double-clicked a placeholder or special text (or selected the Properties popup menu item; in this way, multiple texts can be selected as well).
...
On this tab you can edit the display [properties](Glossary_o_eigenschaften.htm) of text. Such a text can, e.g., be a text which was freely placed on a project page, or it can be a path function text, a designation text of a hyperlink, a dimensioning, a special text, etc.

Overview of the main dialog elements:

Property / Assignment:

The hierarchy [levels](Glossary_o_etagen.htm) Format, Text box, Alignment box, Value / unit, Position and Date / time are available here. These can be opened by clicking the ![](../Pictures/Gui/ALL/all_plussign_as.png) symbol.

![](../Pictures/Gui/ALL/note.png)Notes:

- For dimensionings, hyperlinks, and image files, only the Format hierarchy level is available. For image files, only the [settings](Glossary_o_einstellungen.htm) Layer and Invisible can be modified.
...
For texts from imported EPLAN 5 [projects](Glossary_o_projekte.htm), there are additionally the following special orientations:
...
Choose the entry "Yes" from the drop-down list if the text is not supposed to be visible on the page. It is still present in the project, however. Path [function texts](Glossary_o_funktionstexte.htm) are present in the properties of each [component](Glossary_o_schaltzeichen.htm) and can be displayed for it. In the user settings, you define whether or not invisible elements are to be displayed and / or printed (in a different color).

Row spacing:

Enter the vertical distance between text [lines](Glossary_o_leitungen.htm) in this field, meaning, the distance from the lower edge of a text line to the next lower text line edge. The row spacing will depend on the font chosen. Select an entry from the drop-down list or enter a value manually. Value "1" corresponds to single line spacing, value "2" to double spacing, etc. Single spacing is 1.9 times the font size (height of the upper-case letter "M").

Language:

With this drop-down list you determine in which language the respective property text is to be displayed.

- All displayed languages (arrange vertically): The translated text is shown in all languages that you have selected under the menu items Options > Settings > Projects > "Project name" > Translation > General in the Displayed languages field. The translated texts are arranged vertically in the project pages.
- All displayed languages (arrange horizontally): The translated text is shown in all languages that you have selected under the menu items Options > Settings > Projects > "Project name" > Translation > General in the Displayed languages field. The translated texts are arranged horizontally in the project pages.
...
- One language (variable): The translated text is shown in the language that you have selected under the menu items Options > Settings > Projects > "Project name" > Translation > General.
...
Note that when you [move property texts](gededitgui_h_symboltexteverschieben.htm) using Edit > Text > Move property text the leader line is also moved.
...
If the Activate alignment box check box is activated, you use this drop-down list to specify whether a graphic is displayed and the [form](Glossary_o_verlauf.htm) of this graphic. If you select "No", the alignment box is invisible.
...
If this check box is selected then [user-defined](Glossary_o_benutzerdefiniert.htm) line-breaks are replaced with spaces. All spaces before the line break are deleted and spaces after the line break are retained.
...
| Docking | Meaning |
| Below | If possible, the texts [dock](Glossary_o_andocken.htm) underneath the main text. |
...
This behavior corresponds to the centering of [property texts](Glossary_o_eigenschaftstexte.htm) as described in the section [Display Properties: Placed property](devicetaggui_r_anzeigeeigenschaften.htm#Blockzentriert).
