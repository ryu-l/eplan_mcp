...
You have opened a project. Utilities > Revision control > Property comparison of projects > Compare projects.

In this dialog you can specify [settings](Glossary_o_einstellungen.htm) for the property comparison of [projects](Glossary_o_projekte.htm). To do this you compare specific [properties](Glossary_o_eigenschaften.htm) of the current project with the corresponding properties of a reference project. Normally, the reference project is a copy of an older version of the current project; you can, however, select any project as a reference project.
...
If this check box is selected, then the list will only show write-protected [reference projects](Glossary_o_referenzprojekte.htm) generated from the current project; otherwise, all of the existing projects in the project directory (except for the current one) will appear in the list.
...
If you have selected the entry "-Not activated-", the default scheme is used. In this case, all of the properties of all the [objects](Glossary_o_objekte.htm) will be compared. This can take some time for larger projects.

Structure identifier setting:

This field shows the current scheme for replacing [structure identifiers](Glossary_o_strukturkennzeichen.htm). If you have renamed the structure identifiers, they will normally be treated in the property comparison of projects as if they have been deleted and inserted again. If you do not wish to display such changes, you can [create](Glossary_o_erstellen.htm) a scheme that tells you which structure identifiers were renamed. The specified structure identifiers are no longer seen as different during a property comparison of projects. This setting is handy when you have activated the alternative identification of objects using the check box Alternative identification.
...
Revision markers can only be inserted in a reference project that is not write-protected. A message is displayed if the reference project is write-protected. Before executing the property comparison you must therefore remove the write protection of the reference project. Note that even [temporary reference projects](Glossary_o_temporaerereferenzprojekte.htm) are write-protected.

Delete all existing revision markers:

If this check box is selected, the [revision markers](Glossary_o_revisionsmarkierungen.htm) from a previous property comparison will be deleted from the project.
...
If the check box is not activated, objects are identified using the internal object [ID](Glossary_o_id.htm).

![](../Pictures/Gui/ALL/note.png)Note:

When you compare the properties of projects that were automatically generated (e.g. using the EPLAN Engineering Configuration), you should activate this check box because the internal object IDs are reallocated every time you [generate](Glossary_o_erzeugen.htm) projects. This means that every time you generate them, a completely new project emerges and all objects in this project are given new object IDs. This makes it impossible to carry out a properties comparison using the identification of objects based on the object ID anymore.
...
[Compare properties of projects](revisionmgtgui_h_revvergleichen.htm)
