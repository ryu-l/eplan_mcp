...
The import of [devices](Glossary_o_betriebsmittel.htm) and [routing connections](Glossary_o_verlegeverbindungen.htm) is carried out in two separate steps for which different menu [items](Glossary_o_bauteile.htm) are available:

- Project data > Devices > Import: Imports the device tag list. Devices with [unplaced functions](Glossary_o_nicht_platziertefunktionen.htm) arise in the project. These imported devices with one or more parts are displayed in the device or 3D mounting layout navigator and can be Dragged & Dropped from the [navigators](Glossary_o_navigatoren.htm) in the tree view. To this purpose it has to be either a mechanical [part](Glossary_o_artikel.htm) or the assigned part has to dispose of the required information (dimensions, macro).
- Project data > Connections > Import > Routing connections: Imports the routing [connections](Glossary_o_verbindungen.htm). This creates non-routed routing connections, if both the source and target of the imported routing connections are found. The non-routed routing connections can be rerouted in the connections navigator via the popup menu item Route (layout space).

Correct assignment between the data fields in the data source and the EPLAN [properties](Glossary_o_eigenschaften.htm) is important for successful importing. Such a field assignment is saved in a scheme. A predefined scheme exists, but usually has to be customized in accordance with the respective data source.

### Field assignments as requirement

#### [ClosedMain functions](javascript:void(0);)

The DT (full) ([ID](Glossary_o_id.htm) 20006) property must be assigned to a corresponding data field in the data source so that [main functions](Glossary_o_hauptfunktionen.htm) can be generated during the import of devices.
...
If the DT is the same, terminals and [plugs](Glossary_o_stecker.htm) are differentiated by means of the Terminal / pin designation (ID 20030) property. For the import of these [functions](Glossary_o_funktionen.htm) the Terminal / pin designation (ID 20030) property has to be assigned to a data field in addition to the DT (full) (ID 20006) property.

If parts from the parts management are assigned to the terminals and plugs via the Part number [x] (ID 20100 x) property, the [function definitions](Glossary_o_funktionsdefinitionen.htm) are determined analog to the devices from the parts data. Terminals with parts data are generated as [main terminals](Glossary_o_hauptklemmen.htm) during the import.
...
If the DT (full) (ID 20006) property is assigned additionally, the import automatically generates a [cable](Glossary_o_kabel.htm) connection.

The "Conductor / wire" function definition is assigned by default to the resulting non-routed routing and [cable connections](Glossary_o_kabelverbindungen.htm). This function definition can be modified via a further field assignment (for example with the Function definition (ID 20026) property.)
