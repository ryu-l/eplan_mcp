# Connection planning objects

The connection [planning objects](Glossary_o_planungsobjekte.htm) (piping planning [objects](Glossary_o_objekte.htm), [cable](Glossary_o_kabel.htm) planning objects) are managed in parallel to general planning objects, and behave in a way similar to them. External documents / pages, parts and [function templates](Glossary_o_funktionsschablonen.htm) can be stored at a connection planning object, but not [PLC addresses](Glossary_o_sps_adressen.htm) or [macros](Glossary_o_makros.htm).

Like a connection, a connection planning object must have a source and a target. The specification of the source and target can be carried out in the [property dialog](Glossary_o_eigenschaftendialog.htm) of a piping planning object by using the two fields Source segment and Target segment and in the property dialog of a cable planning object by using the two [properties](Glossary_o_eigenschaften.htm) of the same name.

Connection planning objects can be drawn or placed in the [graphical pre-planning](planninggui_k_grafischevorplanung.htm) as open [polylines](Glossary_o_polylinien.htm) on pre-planning pages. In the process the source and target segment are taken from the start and end point of the polyline from the graphical editor. If a connection planning object is placed on a pre-planning page as a polyline, the source and target segment can no longer be edited in the property dialog. However, the source and / or target of the connection planning object can be modified further by means of a graphical modification of the respective polyline.

By using the menu [items](Glossary_o_bauteile.htm) Project data > Connections > Exchange source and target you can reverse the source and target at selected connection planning objects in the pre-planning navigator. This way the specifications for the source and target [segments](Glossary_o_segmente.htm) in the property dialog and the graphic on the pre-planning pages are adjusted accordingly.

The check run "028026" can be used to check whether the source or target of a connection planning object are missing.

### Cable pre-planning with cable planning objects

The cable planning objects placed in the graphical pre-planning allows you to specify first cable data at an early stage in the planning without a detailed planning already existing. Such a cable pre-planning is, for example, of interest when the cabling of all the power and control technology components are spread across the entire plant like a network.

In order to determine the complete cable lengths later on you enter an indefinite [part](Glossary_o_artikel.htm) at a cable planning object (for example by using the entry "not yet known" for the part number in the first line on the Parts tab) and enter the respective cable length in the Subset / length property of the part reference data.

By using the "Pre-planning: Planning object overview" report you can then determine and report source and target segments, required time (planning / construction), first costing data (prices) and also overall cable lengths.

In the subsequent detailed planning you can assign the cable planning objects to the cables in the schematic. This allows the values entered in the pre-planning to be transferred into the detailed planning, displayed and reported.

See also

[Pre-planning](planninggui_k_start.htm)

[Pre-planning: Principle](planninggui_k_prinzip.htm)

[Pre-planning: Procedure](planninggui_k_vorgehensweise.htm)

[Creating Planning Objects, PCT Loops, PCT Loop Functions, Containers and Connection Planning Objects](planninggui_h_planungsobjekteerstellen.htm)

[Pre-planning: Reports](planninggui_k_auswertungen.htm)

[Piping Definitions in the P&I Diagram](planningri_k_rdp.htm)