...
- You can simply rename components using graphical operations (like deletion, moving, copying, or insertion from [macros](Glossary_o_makros.htm)).
- Devices can easily be composed of multiple [symbols](Glossary_o_symbole.htm) in this way.
...
For [projects](Glossary_o_projekte.htm) conforming to the IEC standard, the Reporting direction property for the plot frame usually has the value "Vertical"; for projects conforming to the [NFPA standard](Glossary_o_nfpa_norm.htm) "Horizontal".
...
If you hold down the [Shift] key while moving a [component](Glossary_o_schaltzeichen.htm), the original full DT will be retained. The identifying device tags of the components existing on the page and affected by this action are also retained. As a result of this action, the displayed DTs of the moved component, and other affected components, may change. The [functions](Glossary_o_funktionen.htm) of a device retain their relationships during such a moving action.

Moving with a pressed [Shift] key furthermore affects the Plug designation and Channel designation [properties](Glossary_o_eigenschaften.htm). During "normal" moving of a component the values of these properties remain unchanged. In the case of moving with a pressed [Shift] key the values of the properties Plug designation and Channel designation are adapted so that the values of the properties Plug designation (automatic) and Channel designation (automatic) remain the same.
