...
A PLC address is usually composed of several address elements. However, the structure of [PLC addresses](Glossary_o_sps_adressen.htm) can vary with the type of PLC. Digital and analog input and output addresses are also differently constructed in many cases.

- Addressing digital inputs and outputs: Digital input and output addresses are usually constructed from a letter (for input or output), a byte address and a bit address. In addition to bit and byte addresses, there are also higher-order addresses such as WORD and DWORD addresses (for [PLC cards](Glossary_o_sps_karten.htm), PLC [modules](Glossary_o_module.htm) etc.).
...
Every address element of a PLC address is represented using a format element of type "Counter". Separators between the counters show the borders of the address elements. Areas with fixed numbers of positions within the address format can be placed after each other without a [separator](Glossary_o_trennzeichen.htm).
...
| N | Station [ID](Glossary_o_id.htm) |
| R | PLC card is placed on [rack](Glossary_o_baugruppentraeger.htm) ID |
...
This results in (e.g.) the creation of the following addresses of the [PLC connection points](Glossary_o_sps_anschluesse.htm) with the identifier "E" (inputs):
