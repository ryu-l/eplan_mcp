...
- You have selected a PLC box in the graphic editor. Edit > Properties. In the Properties <...> dialog, select the PLC box tab.
- You have selected a PLC box in the device navigator or in one of the PLC [navigators](Glossary_o_navigatoren.htm). Popup menu item Properties or Properties (global). In the Properties <...> dialog, select the PLC box tab.

In this tab you specify the [properties](Glossary_o_eigenschaften.htm) for the selected PLC box. (At the main function you specify the properties for the entire PLC card.) In addition to the properties possessed by all [functions](Glossary_o_funktionen.htm), further PLC specific properties are shown here.
...
For [PLC boxes](Glossary_o_sps_kaesten.htm), among other properties, the following are available:
...
- Start address of PLC card: The value entered here specifies the start value for the address range of a PLC card. At the same time the addressing of the inputs and outputs of this card begins with the start value entered here. This value can also be used as the [configuration value](eservicesgui_d_formatzaehler.htm#Konfigurationswert) for readdressing [PLC configurations](Glossary_o_sps_konfigurationen.htm). For a card that has both inputs and outputs, specify the start value of the inputs here.
...
- If the Start address 2 of PLC card property is filled for a PLC box, the values of the properties Start address of PLC card and Start address 2 of PLC card are written to the export file during export of PLC data; even if there are no [PLC connection points](Glossary_o_sps_anschluesse.htm) for the respective address range.

![](../Pictures/Gui/ALL/note.png)Note:

The properties Address range (SIEMENS STEP 7 Classic) and Address range 2 (SIEMENS STEP 7 Classic) are available for [data exchange](Glossary_o_datenaustausch.htm) in the "SIEMENS STEP 7 Classic" format. In these properties enter the size of the address range within the PLC controller that is occupied by the card here, for example "4". Depending on the PLC card used a specification either in bits or bytes is required here. You can use the Address range 2 (SIEMENS STEP 7 Classic) property to specify a separate size for the address range of the outputs. The usage of these properties for the address range is analog to the properties of the start addresses. The above mentioned notes for the start addresses also apply for these properties.
