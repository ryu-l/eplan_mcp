# Dialog Designate connections

[![Closed](../../Skins/Default/Stylesheets/Images/transparent.gif)Dialog call:](javascript:void(0);)

You have selected a connection in the graphical editor, or in the Connections - <Project name> navigator dialog. Or you have selected one or more pages or a project in the Pages - <Project name> navigator dialog. Project data > Connections > Numbering > Enter designation.

In this dialog you can designate those [connection definition points](Glossary_o_verbindungsdefinitionspunkte.htm) that are able to possess a connection designation.

Once you have called up the menu item, the [connections](Glossary_o_verbindungen.htm) are first automatically newly generated.

Overview of the main dialog elements:

Settings:

Select a scheme defining the [settings](Glossary_o_einstellungen.htm) for the [connection numbering](Glossary_o_verbindungsnummerierung.htm) from the drop-down list. Click [...] to open the [Settings: Connection numbering](wirenumberinggui_d_verbnumeinstellungen.htm) dialog. Define the numbering settings here and store them in a scheme.

Start value / increment:

The table lists all [connection groups](Glossary_o_verbindungsgruppen.htm) that are defined in the current setting and contain a counter and /or subcounter in the format. For these groups the counter begins with the specified start value and increases by the specified increment. Both values can be changed in the table.

If the Group check box is activated for a connection group, the counters for the connection group will be combined. In this case, the start value and increment are the same for all the counters / subcounters in a group.  
If the check box is deactivated, you can specify separate values for the start value and increment for each counter / subcounter.

The counter sequence is defined by the sequence of the format groups in the Settings: Connection numbering dialog (Designation tab) and the counter sequence in the groups.

![](../Pictures/Gui/ALL/note.png)Note:

The increment "0" is also permitted; in this case, the counter will not be incremented. The next free value will then be determined according to the setting in the Avoid identical designations field.

Overwrite:

In this drop-down list you specify the connection definition points where the designation should be overwritten:

- All: The [designations](Glossary_o_bezeichnungen.htm) are overwritten for all connection definition points that can possess a connection designation.
- Except for "manually set": Connection designations with the Manually set property will not be overwritten. The designations are overwritten for all other connection definition points that can possess a connection designation. The Manually set property is reported by the connection and not by the connection definition points contained in the selection.
- None: Connections that already have a designation remain unchanged and are not redesignated. All other connections with an empty connection designation or with a designation containing one or more "?" marks will be given a new connection designation.

If the Except for "manually set" and None settings are selected, existing [connection designations](Glossary_o_verbindungsbezeichnungen.htm) are applied. Connections which are added to an existing scope adopt the connection designation of the scope. If more than one connection designation exist, the graphically first used and the other designations are overwritten. For example, connections with the same signal will also be given the same connection designation when being renumbered.

The connection definition points of connections which were excluded from connection numbering by one of these settings adopt the connection designation of the connection below.

![](../Pictures/Gui/ALL/note.png)Note:

If you manually place a connection definition point and assign the Manually set property to it, the Manually set property will be applied for the entire connection. However, the remaining connection [properties](Glossary_o_eigenschaften.htm), i.e., the Connection designation property, will be assigned to the connection from the graphically first connection definition point.

Avoid identical designations:

In this drop-down list you specify whether identical designations are permitted. The following settings are available:

- None: Identical designations are generally permitted.
- In the selection: Each connection in the selection is given a unique connection designation. The connection designations are only unique within the selection, otherwise identical designations are permitted in the project.
- In entire project: Identical designations are avoided in the entire project, i.e. every connection has a unique connection designation within the project.
- Per counter reset range (structure / page): Identical designations are only avoided within the range defined for counter resetting. The connection designations are only unique within a counter reset range, otherwise identical designations are permitted in the project. The counter reset range only affects counters but not subcounters.

![](../Pictures/Gui/ALL/example.png)[![Closed](../../Skins/Default/Stylesheets/Images/transparent.gif)Example:](javascript:void(0);)

You have selected the Per counter reset range (structure / page) setting and the counter reset range is defined so that the counter is reset when the page changes. The connection designations are then unique within a page. However, the same connection designations are allowed on different pages.

If identical designations are avoided, then [formats](Glossary_o_formate.htm) with counters or subcounters will not be given existing designations. Instead, the counter or subcounter will be incremented (except when the same scope is contained in the selection). All of the available connection designations in this case will be taken into consideration, as will those that were assigned in the same numbering scheme in the previous connection groups.

If the format contains counters and subcounters, the counters will be incremented if the subcounter is still at its lowest value. Otherwise the subcounter is incremented.

![](../Pictures/Gui/ALL/example.png)[![Closed](../../Skins/Default/Stylesheets/Images/transparent.gif)Example:](javascript:void(0);)

In the following example, the "General connections" connection group with the "Signal" scope was selected for the numbering. The format for the connection designations contains a counter and a subcounter divided by a period. The connections between H1 and H2 and between H3 and H2 already have connection designations. Now the connections between H4 and H6 and between H5 and H6 are to be numbered. The complete schematic section is marked here.

![](../Pictures/Visualisation/ALL/wirenumberinggui_designation_av.png)

The connection between H4 and H6 is designated with 002.1. Because a 001.1 designation already exists and the subcounter for it is already at is lowest value (namely, 1), the counter is incremented.

The connection between H5 and H6 is designated with 002.3. Because the 002.2 designation already exists, the subcounter is incremented.

![](../Pictures/Visualisation/ALL/wirenumberinggui_nomultipledesignation_av.png)

Multiple designations are allowed within one scope if the format is specified as such and the entire scope is contained in the selection. In this case, the [format elements](Glossary_o_formatelemente.htm) must not come from the individual connection.

![](../Pictures/Gui/ALL/example.png)[![Closed](../../Skins/Default/Stylesheets/Images/transparent.gif)Example:](javascript:void(0);)

In the following example, the "General connections" connection group with the "Signal" scope was selected for the numbering. The format for the connection designations contains a page number and a subcounter (likewise with the "Signal" scope), divided by a hyphen. Now all connections are to be numbered.

![](../Pictures/Visualisation/ALL/wirenumberinggui_designationsignal1_av.png)

Because all connections belong to the same signal, they are given the same connection designation, 1-1.

![](../Pictures/Visualisation/ALL/wirenumberinggui_designationsignal2_av.png)

Visibility:

From this drop-down list select the connections that are to be shown:

- Do not modify: The designations are displayed how they are defined, that is, connection designations defined as hidden will not be displayed. This is the default setting.
- All visible: This setting makes all designations visible, even ones specified as "hidden".
- Once per page and range: This setting switches on visibility for connection designation of the graphically first connection and switches it off for all others with the same scope.

Mark as "manually set":

If this check box is selected, all of the connection definition points that were given a connection designation during connection numbering will be assigned the Manually set property.

See also

[Connection Numbering](wirenumberinggui_k_start.htm)

[Designating Connections](wirenumberinggui_h_verbnumbezeichnen.htm)

[Dialog Designate connections: Preview of result](wirenumberinggui_d_ergebnisbezeichnen.htm)