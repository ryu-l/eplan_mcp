...
```
```
public MDPropertyValue ARTICLE_WIRECROSSSECTION_AND_DIAMETER( 
   int index
) {get; set;}
```
```
...
[Send Feedback](mailto:Support-API@eplan.de?subject=Documentation Feedback: Eplan.EplApi.MasterDatau~Eplan.EplApi.MasterData.MDPartsDatabaseItemPropertyList~ARTICLE_WIRECROSSSECTION_AND_DIAMETER(Int32))