...
| sizefromsettings | ``` Frame size from project settings used? (0=should not be actived, 1=should be active) ``` |
| leaderline | ``` activate leader line (0=should not be active, 1=should be active) ``` |
| donttranslate | ``` don't translate automatically (0=text is translated, 1=text is not automatically translated) ``` |
| linespacing | ``` row spacing in number of lines ``` |
...
[Send Feedback](mailto:Support-API@eplan.de?subject=Documentation Feedback: XGedIaFormatText)