# DMDELETEDOBJECTINFO_EDITINGAREA Property

DMDELETEDOBJECTINFO\_EDITINGAREA Property

Defined working section # 36617. This property isn't indexed.

Overload List

| Overload | Description |
| --- | --- |
| [DMDELETEDOBJECTINFO\_EDITINGAREA(Int32)](topic370.html) | Defined working section # 36617. This property isn't indexed. |
| [DMDELETEDOBJECTINFO\_EDITINGAREA](Eplan.EplApi.DataModelu~Eplan.EplApi.DataModel.DeletedObjectInfoPropertyList~DMDELETEDOBJECTINFO_EDITINGAREA().html) | Defined working section # 36617. |

Remarks

The property is used during the revisioning of defined working sections. Displays the name of the defined working section to which the page belongs in the Deleted pages dialog.

See Also

#### Reference

[DeletedObjectInfoPropertyList Class](Eplan.EplApi.DataModelu~Eplan.EplApi.DataModel.DeletedObjectInfoPropertyList.html)
  
[DeletedObjectInfoPropertyList Members](Eplan.EplApi.DataModelu~Eplan.EplApi.DataModel.DeletedObjectInfoPropertyList_members.html)

---

EPLAN API , 21.05.2020, Â© by EPLAN Software and Service GmbH and Co. KG. All rights reserved.

[Send Feedback](mailto:Support-API@eplan.de?subject=Documentation Feedback: Eplan.EplApi.DataModelu~Eplan.EplApi.DataModel.DeletedObjectInfoPropertyList~DMDELETEDOBJECTINFO_EDITINGAREA)