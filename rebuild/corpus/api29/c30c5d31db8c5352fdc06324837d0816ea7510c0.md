# SwitchLocalPropertyPlacements Method (ViewPart)

SwitchLocalPropertyPlacements Method (ViewPart)

Copies or removes all local ProperyPlacemnets and sets flag UseLocalPropertyPlacements.

Syntax

- [C#](#i-syntax-CS)
- [C++/CLI](#i-syntax-CPP2005)

```
```
public void SwitchLocalPropertyPlacements( 
   bool nUseLocal
)
```
```

```
```
public:
void SwitchLocalPropertyPlacements( 
   bool nUseLocal
)
```
```

#### Parameters

*nUseLocal*
:   If true copies all properties from symbol/symbol reference . Otherwise removes all local properties.

Exceptions

| Exception | Description |
| --- | --- |
| [Eplan.EplApi.Base.LockingException](Eplan.EplApi.Baseu~Eplan.EplApi.Base.LockingException.html) | Thrown when user wants to change PropertyPlacement with default configuration. |

See Also

#### Reference

[ViewPart Class](Eplan.EplApi.DataModelu~Eplan.EplApi.DataModel.ViewPart.html)
  
[ViewPart Members](Eplan.EplApi.DataModelu~Eplan.EplApi.DataModel.ViewPart_members.html)

---

EPLAN API , 21.05.2020, Â© by EPLAN Software and Service GmbH and Co. KG. All rights reserved.

[Send Feedback](mailto:Support-API@eplan.de?subject=Documentation Feedback: Eplan.EplApi.DataModelu~Eplan.EplApi.DataModel.ViewPart~SwitchLocalPropertyPlacements)