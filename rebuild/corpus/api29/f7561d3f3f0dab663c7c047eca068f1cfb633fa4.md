...
```
```
public class InterruptionPoint : FunctionBase, IFunctionBase, IWriteProtection
```
```

```
```
public ref class InterruptionPoint : public FunctionBase, IFunctionBase, IWriteProtection
```
```
...
| Public Method | [PauseWriteProtection](Eplan.EplApi.DataModelu~Eplan.EplApi.DataModel.InterruptionPoint~PauseWriteProtection.html) | Temporarily disables write protection. Note that current write protection |
...
[Send Feedback](mailto:Support-API@eplan.de?subject=Documentation Feedback: Eplan.EplApi.DataModelu~Eplan.EplApi.DataModel.InterruptionPoint)