# EDITINGAREA_CRAFT Property (FunctionPropertyList)

EDITINGAREA\_CRAFT Property (FunctionPropertyList)

Trade (Defined working sections) # 25000. This property isn't indexed, and is read-only.

Overload List

| Overload | Description |
| --- | --- |
| [EDITINGAREA\_CRAFT(Int32)](Eplan.EplApi.DataModelu~Eplan.EplApi.DataModel.FunctionPropertyList~EDITINGAREA_CRAFT(Int32).html) | Trade (Defined working sections) # 25000. This property isn't indexed, and is read-only. |
| [EDITINGAREA\_CRAFT](Eplan.EplApi.DataModelu~Eplan.EplApi.DataModel.FunctionPropertyList~EDITINGAREA_CRAFT().html) | Trade (Defined working sections) # 25000. |

Remarks

This property is read-only..

Can be used in the filter schemes for defined working sections in order to filter pages, functions, and connections by trade. The following values are possible:

0 = Electrical engineering

1 = Fluid power, general

2 = Mechanics

3 = Process engineering.

99 = General.

In this context, the "Fluid power, general" setting includes also the trades Hydraulics, Pneumatics, Cooling, and Lubrication. Pages contain the additional trade setting "General". The "General" trade is the default setting when generating report pages. If you filter defined working sections by certain trades, pages with the "General" trade are output along any other trade.

See Also

#### Reference

[FunctionPropertyList Class](Eplan.EplApi.DataModelu~Eplan.EplApi.DataModel.FunctionPropertyList.html)
  
[FunctionPropertyList Members](Eplan.EplApi.DataModelu~Eplan.EplApi.DataModel.FunctionPropertyList_members.html)

---

EPLAN API , 21.05.2020, Â© by EPLAN Software and Service GmbH and Co. KG. All rights reserved.

[Send Feedback](mailto:Support-API@eplan.de?subject=Documentation Feedback: Eplan.EplApi.DataModelu~Eplan.EplApi.DataModel.FunctionPropertyList~EDITINGAREA_CRAFT)