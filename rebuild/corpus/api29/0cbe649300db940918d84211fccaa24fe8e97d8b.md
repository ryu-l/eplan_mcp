# OnStartInspection Method (PartVerification)

OnStartInspection Method (PartVerification)

Syntax

- [C#](#i-syntax-CS)
- [C++/CLI](#i-syntax-CPP2005)

```
```
public abstract void OnStartInspection( 
   bool bOnline
)
```
```

```
```
public:
abstract void OnStartInspection( 
   bool bOnline
)
```
```

#### Parameters

*bOnline*

See Also

#### Reference

[PartVerification Class](Eplan.EplApi.EServicesu~Eplan.EplApi.EServices.PartVerification.html)
  
[PartVerification Members](Eplan.EplApi.EServicesu~Eplan.EplApi.EServices.PartVerification_members.html)

---

EPLAN API , 21.05.2020, Â© by EPLAN Software and Service GmbH and Co. KG. All rights reserved.

[Send Feedback](mailto:Support-API@eplan.de?subject=Documentation Feedback: Eplan.EplApi.EServicesu~Eplan.EplApi.EServices.PartVerification~OnStartInspection)