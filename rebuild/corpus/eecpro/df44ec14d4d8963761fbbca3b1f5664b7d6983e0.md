...
In the event of success or failure, e-mails can be sent to predetermined addresses. This is only possible when the Send mails on success check box or the Send mails on failure check box is selected and values that are valid for the SMTP server have been entered.
...
### [ClosedJob definition with model in EOX format](javascript:void(0);)
...
### [ClosedJob definition with dynamic specification of the basic model via fileTrigger](javascript:void(0);)
...
### [ClosedJob definition with dynamic specification of the basic model via webserviceTrigger](javascript:void(0);)
...
### [ClosedJob definition with dynamic specification of the Job name via webserviceTrigger](javascript:void(0);)
...
If the Maximum number of parallel Jobs is set to a value greater than 1 in the settings, several Jobs can be executed parallel. However, for specific Jobs, parallel execution cannot be allowed and it must be possible to exclude it.
...
### [ClosedJob definition with workerMutex](javascript:void(0);)
...
### [ClosedJob definition with globalMutex](javascript:void(0);)
