...
### [ClosedExample code of valid clipboard contents](javascript:void(0);)
...
| height | optional | Integer, fillForm |  | Height of the entire table. Integer = height of the table in pixel fillForm = the table occupies the entire height of the Form-UI |
...
| width | optional | Integer, fitToForm |  | Width of the entire table. Integer = Width of the table in pixels fitToForm = Table takes on the entire width of the Form-UI |
...
The following example shows a table which representation is limited to a maximum of 5 rows and 7 columns. One of the 7 columns is fixed on the left-hand side and 2 columns are fixed on the right-hand side. The columns and rows in between are reached by means of scrollbars.
